...
binsX = list(np.arange(float(Xdim[0]),(float(Xdim[1]) + binspace),binspace))
binsX.insert(len(binsX),int(infinity))
binsX = np.array(binsX)
binsY = list(np.arange(float(Ydim[0]),(float(Ydim[1]) + binspace),binspace))
binsY.insert(len(binsY),int(infinity))
binsY = np.array(binsY)
hist2D, hist_edgesX, hist_edgesY = np.histogram2d(df_rmsd_rg["rmsd"].values.tolist(),df_rmsd_rg["rg"].values.tolist(),bins = (binsX, binsY),weights=None)
